...
struct BackupRepositoryTests {
    @Test @MainActor
    func backupDocumentAcceptsCurrentAndPreRenameTypes() {
        let identifiers = Set(
            RedeemDeckBackupDocument.readableContentTypes.map(\.identifier)
        )

        #expect(identifiers.contains("app.pythonide.redeemdeck.backup"))
        #expect(identifiers.contains("app.pythonide.codevault.backup"))
    }
    ...
    func restoreBuildsCategoriesForSchemaOneBackups() async throws {
        ...
        let legacy = RedeemDeckBackupArchive(
            schemaVersion: 1,
