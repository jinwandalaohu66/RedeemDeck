...
struct CodeRetrievalRepositoryTests {
    ...
    func retrievalIsAllOrNothingWhenQuantityExceedsInventory() async throws {
        ...
        let repository = RedeemDeckRepository(modelContainer: fixture.container)

        await #expect(throws: RedeemDeckRepositoryError.self) {
    ...
    func pendingRetrievalSurvivesRepositoryRecreation() async throws {
        ...
        let firstRepository = RedeemDeckRepository(modelContainer: fixture.container)
        let selection = try await firstRepository.reserveCodes(
            categoryID: fixture.categoryID,
            quantity: 2
        )

        let reopenedRepository = RedeemDeckRepository(modelContainer: fixture.container)
    ...
    func migrationMakesAnOlderReservationResumable() async throws {
        ...
        try await RedeemDeckMigrationService(modelContainer: fixture.container)
            .performAdditiveMigration()
        let pending = try await RedeemDeckRepository(modelContainer: fixture.container)
