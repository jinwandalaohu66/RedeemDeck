...
| Get codes | App row → native quantity `Alert` → `CodeRetrievalView` | `RedeemDeckRepository.reserveCodes` + persisted retrieval ID | atomic quantity, category isolation, earliest-expiry, and UI tests |
...
| Code management | App trailing swipe → `AppCodeManagementView` | `DashboardRepository` + `RedeemDeckRepository` | native swipe and typed navigation UI tests |
...
| Code details | Code browser row → `CodeDetailView` | immutable `CodeRowSummary` + `RedeemDeckRepository` | direct navigation, lifecycle, and UI tests |
...
- Sent means the code left local inventory. RedeemDeck does not claim to detect whether App Store redemption actually occurred.
...
New exports use the `app.pythonide.redeemdeck.backup` type and `.redeemdeckbackup` extension. The isolated `app.pythonide.codevault.backup` import type and `.codevaultbackup` extension exist only so backups created before the product rename can still be selected and decoded. New files are never exported with the upstream identifier.
...
4. Remove the upstream backup import type only after supported upgrade paths no longer include pre-rename exports.
