...
struct BackupRestoreView: View {
    ...
    @State private var exportDocument = RedeemDeckBackupDocument()
    @State private var exportFilename = "RedeemDeck Backup"
    ...
    private func prepareExport() {
        ...
        Task {
            do {
                ...
                exportDocument = RedeemDeckBackupDocument(data: data)
                exportFilename = "RedeemDeck \(Date().formatted(.iso8601.year().month().day()))"
                isExporting = true
            } catch {
...
struct RestorePreview: Identifiable {
    let id = UUID()
    let archive: RedeemDeckBackupArchive
}
