...
nonisolated enum BackupCodecError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .unsupportedVersion: String(localized: "This backup was created by an unsupported version of CodeVault.")
        case .invalidArchive: String(localized: "The selected file is not a valid CodeVault backup.")
        }
    }
}
...
actor BackupCodec {
    ...
    func encode(_ archive: CodeVaultBackupArchive) throws -> Data {
    ...
    func decode(_ data: Data) throws -> CodeVaultBackupArchive {
        guard data.count <= maximumFileSize else {
            throw BackupCodecError.fileTooLarge
        }
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        guard let archive = try? decoder.decode(CodeVaultBackupArchive.self, from: data) else {
            throw BackupCodecError.invalidArchive
        }
        guard (1...CodeVaultBackupArchive.currentVersion).contains(archive.schemaVersion) else {
